...
inline int tail(int x) { return (x + 1) * sqr; }
inline int head(int x) {  return x * sqr + 1;  }
...
int query(int l,int r) {
    int ans = 0;
    int x = index(l), y = index(r);
    if(x == y) {
        for(int i = l ; i <= r ; ++i) ans += bool(cnt[i]);
    } else {
        for(int i = tail(x) ; i >= l ; --i) ans += bool(cnt[i]);
        for(int i = head(y) ; i <= r ; ++i) ans += bool(cnt[i]);
...
inline void add(int x) { if(!cnt[x]++) ++sum[index(x)]; }
inline void del(int x) { if(!--cnt[x]) --sum[index(x)]; }
