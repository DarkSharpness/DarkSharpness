...
int g[N]; // g[i] := gcd in [l,i]
int ans1[N],ans2[N];

// search in [l,r] , find first no less than v
inline int lower_bound(int l,int r,int v) {
    while(l < r) {
        int mid = (l + r) >> 1;
        if(g[mid] > v) l = mid + 1;
        else           r = mid;
...
signed main() {
    ...
    for(int l = n ; l ; --l) { // g[i] := gcd in [l,i]
        int gg   = g[l] = a[l];
        int last = g[l];
        for(int i = l + 1 ; i <= n ; ++i) { // Amortized O(nlogn)
            if(!(gg % g[i])) break;
            if(last == g[i]) { g[i] = gg ; continue; }
            last = g[i];
            gg   = g[i] = gcd(gg,g[i]); // no bigger than g[i] / 2
        }
        for(int i = head[l] ; i ; i = e[i].nxt) {
            int r = e[i].to;
            int q = e[i].i;
            ans1[q] = g[r];
            ans2[q] = lower_bound(r + 1,n + 1,g[r] - 1) - lower_bound(l,r,g[r]) - 1;
        }
    }
