...
void dfs(int x) {
    if(dark::read(a[x])) return void(siz[x] = 1);
    dfs(l[x] = ++tot); siz[x] += siz[l[x]];
    dfs(r[x] = ++tot); siz[x] += siz[r[x]];
    if(siz[l[x]] > siz[r[x]]) swap(l[x],r[x]);
}

void clean(int x) {
    if(a[x]) return remove(a[x]);
    clean(l[x]); clean(r[x]);
}

void updat(int x) {
    if(a[x]) return insert(a[x]);
    updat(l[x]); updat(r[x]);
}

ll getsum(int x) {
    if(a[x]) return query(a[x]);
    return getsum(l[x]) + getsum(r[x]);
}

void work(int x,bool tag) {
    if(a[x]) return tag ? updat(x) : void();

    work(l[x],0); /* Light son. */
    work(r[x],1); /* Heavy son. */

    ll tmp = getsum(l[x]);
    ans += min(tmp,ll(siz[l[x]]) * siz[r[x]] - tmp);

    return tag ? updat(l[x]) : clean(r[x]);
}
